...
* Lecture: 20
  ...
  - Classify Simulation models
    ...
    - *Stochastic simulation*: is where the answer will different run to run because there's an element of
  ...
  - *Dynamic models*: in dynamic situation time plays a role
    ...
    - *Queues network mode*: queues is a word for lines, evolve over time
      ...
      - They are particularly for thing related to scheduling and planing
  ...
  - *Continuous models*: are is the mathematical practice of applying a model to continuous data
  ...
  - *Monty carlo simulation*: is a computerized mathematical technique that allows people to account for risk in
    ...
    - *X range*: returns an iterator and only keeps one number in memory at a time

* Lecture: 21

  - If answer is statically valid, it does not mean it's the right answer
