...
* Lecture: 2
  ...
  - Type Conversion: is convert data type
    - e.g
      #+BEGIN_SRC python
      str(3) + 'bf'  # it prints 3bf
      #+END_SRC

  - Type Checking: Python checks the type of the operands before it applied things
  - Type Checking fall on a specturm from wheak to strong typing

** Operator Precedence
   - Exponentiation are done before you do multiplication & divison done before you do addirion & subtraction
   - If use /parentheses/ the culculation heppened in parentheses before the other operator
     e.g
     #+BEGIN_SRC python
  ...
  - *Variables*: has own values
    - They are in assignment statement
    - /Nuance/ contain all possible values
      e.g
      #+BEGIN_SRC python
    ...
    - Use variable any place where wanna use value
    - Use variable name that make sense

  - *Colon*: Identifying a block of code

  - *Statements*: are legal commands that python can interpret such as print, assignment

  -*Straight line program*: that is, a program in which execute in the sequence of the instruction one by one

  - *Branching programs* is something that can change the order of instructions based on some test
    ...
    - Indentation matters
      ...
      if 'd' < x:
        print('Hello')
      print('nope') # if does the test
      # but if statement isn't true and print has same indentation as the if so the secound print statement
      #+END_SRC
